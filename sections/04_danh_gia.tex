...
\caption{Verification Dashboard sau khi áp dụng một tamper scenario: mục ``Sự Kiện Phát Hiện Giả Mạo'' ghi nhận outcome \texttt{RECORD\_\allowbreak COUNT\_\allowbreak MISMATCH}, cùng bảng ``Luồng Dữ Liệu'' hiển thị các lineage event Source$\to$Bronze$\to$Silver$\to$Gold với số bản ghi vào/ra. Panel ``Block Bị Phá Vỡ Đầu Tiên'' chưa hiển thị dữ liệu trong export này; phép JOIN chi tiết giữa first broken block và lineage event cần thực hiện qua KPI~6 (\texttt{sql/dashboard\_queries.sql}).}
